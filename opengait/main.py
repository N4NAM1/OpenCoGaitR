import os
# 🔥 [GaitCIR] 必须放在最前面：配置 HF 镜像，防止 CLIP 下载超时
...
parser = argparse.ArgumentParser(description='Main program for OpenGait (GaitCIR Edition).')
...
def run_model(cfgs, training):
    ...
    # 3. [GaitCIR] CLIP 通常冻结了大部分参数，fix_BN 逻辑可能不适用或需谨慎开启
...
#main:don't forget nccl or gloo
if __name__ == '__main__':
    ...
    # 使用 try-finally 结构，确保报错也能清理
    try:
        initialization(cfgs, training)
        run_model(cfgs, training)
    except Exception as e:
        print(f"❌ [Error] Program crashed: {e}")
        raise
    finally:
        # 🔥 核心修复：必须显式销毁进程组，释放共享内存
